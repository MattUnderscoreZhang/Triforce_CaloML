...
ele_path = "/data/LCD/NewSamples/RandomAngle/CLIC/GammaEscan_RandomAngle_MERGED/*.h5"
chpi_path = "/data/LCD/NewSamples/RandomAngle/CLIC/Pi0Escan_RandomAngle_MERGED/*.h5"
ele_label = "Gamma"
chpi_label = "Pi0"
...
for (feature, max_bin, bin_step) in features:
    ...
    plt.hist(np.clip(combined_ele_data[feature], bins[0], bins[-1]), bins=bins, normed=True, histtype='step', linewidth=1, label=ele_label)
    plt.hist(np.clip(combined_chpi_data[feature], bins[0], bins[-1]), bins=bins, normed=True, histtype='step', linewidth=1, label=chpi_label)
