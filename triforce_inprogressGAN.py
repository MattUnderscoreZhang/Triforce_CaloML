...
class DiscriminatorTrainer:
    ...
    def discriminator_eval(self,event_data, do_training=False):
        if do_training:
            discriminator.net.train()
        else:
            discriminator.net.eval()
        outputs = discriminator.net(event_data)
        return_event_data = {}
        # classification
        truth_class = Variable(event_data["classID"].cuda())
        class_reg_loss = discriminator.lossFunction(outputs, event_data, options['lossTermWeights'])
        if do_training:
            microbatch_norm_loss = {}
            for key in class_reg_loss:
                microbatch_norm_loss[key] = class_reg_loss[key] / options['nMicroBatchesInMiniBatch']
...
class_reg_trainer = ClassRegTrainer()
...
def update_batch_history(data, train_or_test, minibatch_n):
    if train_or_test == TRAIN:
        eval_results = class_reg_trainer.class_reg_train(data)
    else:
        eval_results = class_reg_trainer.class_reg_eval(data)
...
def update_GAN_history(data, train_or_test, minibatch_n):
    # if train_or_test == TRAIN:
        # eval_results = class_reg_trainer.class_reg_train(data)
    # else:
        # eval_results = class_reg_trainer.class_reg_eval(data)
...
def GAN_training():
    ...
    for _ in range(n_GAN_training_batches):

        for _ in range(2):

            # generate fake data and mix with real data
            for _ in range(n_events_discriminator_batch):
                try:
                    real_batch += next(trainLoader)
                catch EndException:
                    trainLoader = (reinit)
                    real_batch += next(trainLoader)
            ...
            total_GAN_data = real_batch+fake_batch
            total_GAN_labels = real_labels+fake_labels
            total_GAN_data_with_labels = shuffle(zip(total_GAN_data, total_GAN_labels))

            # use data to train discriminator
            for data in total_GAN_data_with_labels:
                discriminator_trainer.reset()
                update_GAN_history(data, train_or_test, minibatch_n)
                print('-------------------------------')
                print('epoch %d, batch %d' % (epoch+1, minibatch_n))
                print_GAN_stats(BATCH)
        
        # train generator
        for _ in range(n_events_generator_batch):
            fake_batch += generator.generate()
            generated_labels = torch.zeros()
            eval_results = discriminator_trainer.discriminator_eval(fake_batch)
            generator.loss_function(generated_labels, eval_results['discrimination_labels'])
